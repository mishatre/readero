...
const SettingsProvider = ({ children }: ISettingsProviderProps) => {
    ...
    useEffect(() => {
        ...
        const {wordsPerMinute: defaultWordsPerMinute, ...defaultSettingsRest} = defaultSettings;

        const settingsString = localStorage.getItem('settings');        
        if(!settingsString) {
            localStorage.setItem('settings', JSON.stringify(defaultSettingsRest));
        ...
        if(!wordsPerMinuteString) {
            localStorage.setItem('wordsPerMinute', String(defaultWordsPerMinute));
    ...
    const setSetting = useCallback(<K extends keyof ISettings>(key: K, value: ISettings[K] ) => {
        setSettings((currentSettings) => {
            ...
            if(key === 'wordsPerMinute') {
                ...
                localStorage.setItem('settings', JSON.stringify({...currentSettings, [key]: value }));
            }
            return {...currentSettings, [key]: value};
        });
    }, []);

    const setWordsPerMinute      = useCallback((newValue: number ) => setSetting('wordsPerMinute',      newValue), [setSetting]);
    const setFont                = useCallback((newValue: string ) => setSetting('font',                newValue), [setSetting]);
    const setFontSize            = useCallback((newValue: number ) => setSetting('fontSize',            newValue), [setSetting]);
    const setUseORP              = useCallback((newValue: boolean) => setSetting('useORP',              newValue), [setSetting]);
    const setSlowDownOnLongWords = useCallback((newValue: boolean) => setSetting('slowDownOnLongWords', newValue), [setSetting]);
